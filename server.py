...
军机处模式移植（junjichu/server.py，姊妹项目实测过的坑都规避）：
...
- 口令门（任务单 07）：data/config.json 顶层 {"code":"xxxxxx"} 启用——所有 /api/*
  ...
  限速（R9-F7，军机处口径）：同 IP 连错 5 次锁 2 分钟（锁定期对口令也 429），
  输对清零、超期自动解
...
# ------------------------------------------------------------------
# 口令门限速（R9-F7，照军机处 LAN 令牌口径）：同 IP 连错 5 次锁一段时间。
# 桌面单机的口令门是隐私门不是加密门，但 SUANJIAN_LAN=1 时同网设备可在线
# 穷举 6 位口令（10^6 量级可行）——限速把在线暴力试错的空间掐掉。
# 军机处先例是 5 错锁 10 分钟；本机场景用户自己手滑也计入，时限收窄到 2 分钟。
# ------------------------------------------------------------------
